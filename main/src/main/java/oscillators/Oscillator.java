...
{
    private static final Logger LOG = LoggerFactory.getLogger(Oscillator.class);
    ...
    public final void portamento(int millis)
    {
        if (millis <= 0)
        {
            LOG.error("Oscillator.portamento: The portamento speed must be greater than zero.");
        }
        port = true;
        portSpeed = millis;
    }
    ...
    private void stepPan()
    {
        if (pan != newPan)
        {
            if (pan < newPan)
                ...
                pan -= panAmpStep;
            if (Math.abs(pan - newPan) < panAmpStep)
                pan = newPan;
        }
    }
    ...
    private void stepAmp()
    {
        if (amp != newAmp)
        {
            if (amp < newAmp)
                ...
                amp -= panAmpStep;
            if (Math.abs(amp - newAmp) < panAmpStep)
                pan = newPan;
        }
    }
