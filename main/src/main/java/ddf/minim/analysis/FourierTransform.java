...
public abstract class FourierTransform
{
     ...
     * @example Analysis/FFT/Windows
     * @related <a href=
     *          "http://en.wikipedia.org/wiki/Window_function#Bartlett_window_.28zero_valued_end-points.29">Bartlett
     *          window</a>
     * @related WindowFunction
     */
     ...
     * 
     * @example Analysis/FFT/Windows
     * @related <a href="http://en.wikipedia.org/wiki/Window_function#Bartlett.E2.80.93Hann_window">Bartlett-Hann
     *          window</a>
     * @related WindowFunction
     */
    ...
    FourierTransform(int ts, float sr)
    {
        timeSize = ts;
        sampleRate = (int) sr;
        bandWidth = (2f / timeSize) * (sampleRate / 2f);
        noAverages();
        allocateArrays();
    ...
    public void linAverages(int numAvg)
    {
        ...
        {
            LOG.error("The number of averages for this transform can be at most "
                    + spectrum.length / 2 + ".");
            return;
        }
        else
    ...
    public void forward(float[] buffer, int startAt)
    {
        ...
        {
            LOG.error("FourierTransform.forward: not enough samples in the buffer between " +
