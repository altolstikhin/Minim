...
{
    private static final Logger LOG = LoggerFactory.getLogger(GaussWindow.class);
    double alpha;
    ...
    public GaussWindow(double alpha)
    {
        ...
        {
            LOG.error("Range for GaussWindow out of bounds. Value must be <= 0.5");
            return;
        }
        this.alpha = alpha;
    }
    ...
    protected float value(int length, int index)
    {
        return (float) Math.pow(Math.E,
                -0.5 * Math.pow((index - (length - 1) / (double) 2) / (this.alpha * (length - 1) / 2), 2));
    }
