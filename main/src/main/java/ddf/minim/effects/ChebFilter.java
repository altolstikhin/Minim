...
public class ChebFilter
        extends IIRFilter
{
    private static final Logger LOG = LoggerFactory.getLogger(ChebFilter.class);
    ...
    public void setType(int t)
    {
        if (t != LP && t != HP)
        {
            LOG.error("Invalid filter type, defaulting to low pass.");
            t = LP;
        }
    ...
    public void setPoles(int p)
    {
        if (p < 2)
        {
            LOG.error("ChebFilter.setPoles: The number of poles must be at least 2.");
            return;
        }
        if (p % 2 != 0)
        {
            LOG.error("ChebFilter.setPoles: The number of poles must be even.");
            return;
        }
        if (p > 20)
        {
            LOG.error("ChebFilter.setPoles: The maximum number of poles is 20.");
        }
        poles = p;
        calcCoeff();
    }
