...
public class LowPassFS
        extends IIRFilter
{
    private static final Logger LOG = LoggerFactory.getLogger(LowPassFS.class);
    ...
    public boolean validFreq(float f)
    {
        if (f < 60)
        {
            LOG.error("This filter quickly becomes unstable below 60 Hz, setting frequency to 60 Hz.");
            return false;
        }
        return true;
    }
