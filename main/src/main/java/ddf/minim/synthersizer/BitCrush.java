...
{
    private static final Logger LOG = LoggerFactory.getLogger(BitCrush.class);
     ...
     * the AbstractSynthersizer itself will accomplish the same thing.
     * 
     * @related BitCrush
     */
    ...
    /**
     * Control the bit resolution with another AbstractSynthersizer by patching to bitRes. Values that
    ...
    /**
     * Control the bit rate with another AbstractSynthersizer by patching to bitRate.
    ...
    protected void uGenerate(float[] out)
    {
        if (sampleCounter <= 0)
        {
            ...
            {
                LOG.error("BitCrush audio has " + audio.getLastValues().length + " channels and sampledFrame has "
                        + channelCount());
            }
